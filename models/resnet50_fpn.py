...
from land_ml.models import resnet50  # noqa: E402
from land_ml.utils import torchutils  # noqa: E402
...
class Net(pl.LightningModule):
    ...
    def __init__(self, num_classes, pretrained=True, first_trainable=0, reduced_k_size=False):
        super(Net, self).__init__()

        self.num_classes = num_classes
        self.first_trainable = first_trainable
        if reduced_k_size:
            self.resnet50 = resnet50.resnetk3(
    ...
    def _upsample_cat(self, x, y):
        ...
        """
        _, _, H, W = y.size()
        upsampled_x = F.interpolate(x, size=(H, W), mode="nearest")
...
class CAM(Net):
    ...
    def forward(self, x):
        ...
        cams = [
            self._compute_cam(p, self.classifier.weight[:, index]) for index, p in enumerate(p_list)
        ]
        strided_up_size = get_strided_up_size(image_size, 16)
        unsqueezed_cams = [c.unsqueeze(dim=0) for c in cams]
        interpolated_cams = [
            F.interpolate(uc, strided_up_size, mode="bilinear", align_corners=False)
            for uc in unsqueezed_cams
        ]
