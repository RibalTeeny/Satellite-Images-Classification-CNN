...
from land_ml.utils.decorators import proxy_setup
...
class ResNetKernel3(pl.LightningModule):
    def __init__(self, block, layers, strides=(2, 2, 2, 2), dilations=(1, 1, 1, 1)):
        self.inplanes = 64
        super(ResNetKernel3, self).__init__()
...
def resnet50(pretrained=True, **kwargs):
    model = ResNet(Bottleneck, [3, 4, 6, 3], **kwargs) # always initialize
    if pretrained:
        print("pretrained")
        ...
        # modify_state_dict()
        model.load_state_dict(state_dict) # replace what you can (strict = False)
        
    return model

@proxy_setup
def resnetk3(pretrained=True, **kwargs):
    model = ResNetKernel3(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        print("pretrained")
        state_dict = model_zoo.load_url(model_urls["resnet50"])
        state_dict.pop("fc.weight")
        state_dict.pop("fc.bias")
        state_dict["conv1.weight"] = model.state_dict()["conv1.weight"]
